...
(function () {
  ...
  // STRATEGIA D — profondità dell'inchiostro: il nucleo scuro di una cella
  // colorata deve essere DAVVERO scuro rispetto alla carta (inchiostro),
  // non solo "un po' meno chiaro" (ombra su superficie bianca, sfumatura
  // grigio-su-grigio di uno schermo). Media del nucleo scuro / bianco di
  // riferimento: la stampa reale sta sotto ~0.65; ombre e sfumature morbide
  // restano appena sotto la soglia di classificazione (~0.75-0.82).
  const INK_MAX_MEAN_RELATIVE = 0.72;

  // STRATEGIA E — bordi taglienti: ogni run di celle colorate deve avere,
  // su almeno uno dei suoi confini, una transizione di luminosità RIPIDA
  // (bordo stampato: il passaggio carta->colore avviene in 1-2 px di
  // analisi; un'ombra ci mette decine di px). Salto minimo richiesto su
  // una finestra di 2 px, relativo alla carta.
  const EDGE_MIN_STEP_RELATIVE = 0.12;
  const EDGE_SEARCH_HALF_WINDOW = 12; // px di analisi attorno al confine del run

  // STRATEGIA F — contesto per colonna: frazione massima di celle colorate
  // che possono avere scuro SIA sopra SIA sotto. Le fusioni verticali
  // legittime della stampa sono una minoranza delle celle di una riga;
  // un oggetto solido (mousepad, scatola) ce l'ha sul 100% delle sue celle.
  const COLUMN_CONTEXT_BAD_MAX_FRACTION = 0.34;
  const COLUMN_CONTEXT_MIN_WHITE = 0.5; // frazione di bianco per dire "carta sopra/sotto questa cella"
  ...
  function analyzeDarkRegion(imageData, cell, whiteRef) {
    ...
    if (count > 1) {
      coreMean = sum / count;
      darkStd = Math.sqrt(Math.max(0, sumSq / count - coreMean * coreMean));
    } else if (count === 1) {
      coreMean = sum;
    }
    return { darkFraction, darkStd, coreMean };
  }
  ...
  function classifyCells(cells, whiteRef, imageData) {
    return cells.map((cell) => {
      ...
      if (looksNonWhite) {
        const { darkFraction, darkStd, coreMean } = analyzeDarkRegion(imageData, cell, whiteRef);
        const darkPartIsSolid = darkStd <= DARK_REGION_MAX_STD;
        const enoughDark = darkFraction >= DARK_FRACTION_MIN;
        // STRATEGIA D: il nucleo scuro deve essere inchiostro PROFONDO
        // rispetto alla carta. Ombre su superfici bianche e sfumature
        // grigio-su-grigio restano appena sotto la soglia di classificazione
        // e vengono smascherate qui.
        const inkDeep = whiteRef > 0 && coreMean / whiteRef <= INK_MAX_MEAN_RELATIVE;
        isColored = enoughDark && darkPartIsSolid && inkDeep;
        isClutter = !isColored;
      }
  ...
  function analyzeFrame(videoEl) {
    ...
    if (rawConfidence > 0 && result.whiteRef > 0) {
      ...
      // STRATEGIA F: contesto verificato PER COLONNA, non in media globale.
      // Chiude la falla della scena metà-carta/metà-oggetto: la banda
      // globale passa grazie alla metà bianca, ma sopra e sotto le celle
      // dell'oggetto c'è solo oggetto.
      if (rawConfidence > 0) {
        const badFraction = columnContextBadFraction(
          result.classified, topBand, bottomBand, result.whiteRef,
          ANALYSIS_WIDTH, ANALYSIS_HEIGHT
        );
        if (badFraction > COLUMN_CONTEXT_BAD_MAX_FRACTION) {
          rawConfidence = 0;
        }
      }
    }
  ...
  /**
   * STRATEGIA E — bordi taglienti.
   * Profilo di luminosità per colonna (media verticale della striscia),
   * poi per ogni RUN di celle colorate consecutive si verifica che almeno
   * uno dei suoi due confini presenti una transizione ripida: il bordo
   * stampato passa da carta a colore in 1-2 px di analisi, un'ombra o una
   * sfumatura ci mette decine di px. I run senza alcun bordo netto vengono
   * riclassificati come clutter (non suonano, penalizzano la confidenza).
   */
  function computeColumnProfile(imageData, width, height) {
    const data = imageData.data;
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let y = 0; y < height; y++) {
        const idx = (y * width + x) * 4;
        sum += Math.max(data[idx], data[idx + 1], data[idx + 2]) / 255;
      }
  ...
  function boundaryHasSharpEdge(profile, boundaryX, whiteRef) {
    const width = profile.length;
    const x0 = Math.max(0, Math.round(boundaryX) - EDGE_SEARCH_HALF_WINDOW);
    const x1 = Math.min(width - 3, Math.round(boundaryX) + EDGE_SEARCH_HALF_WINDOW);
    const minStep = EDGE_MIN_STEP_RELATIVE * whiteRef;
    for (let x = x0; x <= x1; x++) {
      if (Math.abs(profile[x + 2] - profile[x]) >= minStep) return true;
    }
    return false;
  }

  function applyEdgeSharpnessGate(classified, profile, whiteRef, width) {
    const pitch = width / EXPECTED_PIXEL_COUNT;
    let i = 0;
    while (i < classified.length) {
      if (!classified[i].isColored) { i++; continue; }
      // Trova il run di celle colorate consecutive [i, j)
      let j = i;
      while (j < classified.length && classified[j].isColored) j++;

      const leftBoundary = i * pitch;
      const rightBoundary = j * pitch;
      // Un confine che coincide col bordo della finestra non è verificabile:
      // conta come "non verificato", non come bocciato.
      const leftCheckable = i > 0;
      const rightCheckable = j < classified.length;
      const leftSharp = leftCheckable ? boundaryHasSharpEdge(profile, leftBoundary, whiteRef) : false;
      const rightSharp = rightCheckable ? boundaryHasSharpEdge(profile, rightBoundary, whiteRef) : false;

      const anyCheckable = leftCheckable || rightCheckable;
      if (anyCheckable && !leftSharp && !rightSharp) {
        for (let k = i; k < j; k++) {
          classified[k].isColored = false;
          classified[k].isClutter = true;
          classified[k].contrast = 0;
        }
      }
      i = j;
    }
  ...
  /**
   * STRATEGIA F — contesto per colonna.
   * Per ogni cella COLORATA, misura la frazione di bianco nelle bande di
   * contesto sopra e sotto limitatamente all'intervallo x DI QUELLA CELLA.
   * Una cella con scuro sia sopra che sotto è "cattiva": può essere una
   * fusione verticale legittima della stampa (minoranza) o un oggetto
   * solido (mousepad, scatola: il 100% delle sue celle è cattivo).
   * Ritorna la frazione di celle colorate cattive. Pura e testabile.
   */
  function columnContextBadFraction(classified, topBand, bottomBand, whiteRef, width, bandHeight) {
    const coloredCells = classified.filter((c) => c.isColored);
    if (coloredCells.length === 0) return 0;

    const threshold = whiteRef * COLORED_VALUE_RATIO;
    const pitch = width / EXPECTED_PIXEL_COUNT;

    function columnWhiteFraction(band, x0, x1) {
    ...
    for (const cell of coloredCells) {
      const x0 = Math.max(0, Math.round(cell.index * pitch));
      const x1 = Math.min(width - 1, Math.round((cell.index + 1) * pitch) - 1);
      const topWhite = columnWhiteFraction(topBand, x0, x1);
      const bottomWhite = columnWhiteFraction(bottomBand, x0, x1);
      if (topWhite < COLUMN_CONTEXT_MIN_WHITE && bottomWhite < COLUMN_CONTEXT_MIN_WHITE) {
        badCount++;
      }
    }

    return badCount / coloredCells.length;
  }
  ...
  function analyzeImageData(imageData, width, height) {
    ...
    // STRATEGIA E: i run di celle colorate senza alcun bordo netto ai
    // confini vengono riclassificati come clutter PRIMA del calcolo della
    // confidenza, così pesano contro la scena invece che a suo favore.
    const profile = computeColumnProfile(imageData, width, height);
    applyEdgeSharpnessGate(classified, profile, whiteRef, width);
  ...
  window.GridDetector = {
    ...
    columnContextBadFraction, // esposto per testabilità (strategia F)
    reset,
