...
(function () {
  ...
  // Palette allineata al tema chiaro: maschera BIANCA traslucida (il nero
  // diventa bianco), accenti nel nuovo arancione #E57D4A = rgb(229,125,74).
  const MASK_COLOR = 'rgba(255, 255, 255, 0.72)';
  const WINDOW_BORDER_COLOR = 'rgba(229, 125, 74, 0.9)';
  const SEGMENT_MATCHED_COLOR = 'rgba(229, 125, 74, 0.9)';
  const SEGMENT_INTERPOLATED_COLOR = 'rgba(229, 125, 74, 0.45)';
  const PLAYHEAD_COLOR = 'rgba(229, 125, 74, 0.75)'; // arancione UI (#E57D4A), pieno ma
  ...
  function init(canvasEl, statusEl) {
    ...
    // Idempotente: init viene richiamata a ogni ingresso in lettura
    // (la lettura è chiudibile e riavviabile), i listener vanno
    // registrati una volta sola.
  ...
  function drawDetectedBlobs(blobs, analysisWidth, analysisHeight, windowRect) {
    ...
    ctx.fillStyle = 'rgba(229, 125, 74, 0.85)';
  ...
  function render(frame) {
    ...
    // 4. Bordo della finestra: PULSA molto dolcemente durante la ricerca
    // (da più trasparente a più acceso, ciclo ~2.4s) e resta STATICO e
    // pieno quando la fascia è agganciata — è il linguaggio di stato
    // primario, leggibile con la coda dell'occhio senza testo.
    let borderAlpha = 0.9;
    if (!frame.showDetection) {
      const phase = (performance.now() % 2400) / 2400; // ciclo di 2.4s
      const wave = 0.5 + 0.5 * Math.sin(phase * Math.PI * 2);
      borderAlpha = 0.2 + 0.6 * wave; // respira tra 0.2 e 0.8
    }
    ctx.strokeStyle = `rgba(229, 125, 74, ${borderAlpha.toFixed(3)})`;
    ctx.lineWidth = 4;
