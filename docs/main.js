...
(function () {
  ...
  function stateLabelText(confidenceState) {
    const STATES = window.ConfidenceState.STATES;
    // Durante la lettura (LOCKED) nessuna scritta: l'etichetta compare solo
    // negli stati di attesa, dove dà un feedback utile.

    if (confidenceState === STATES.SEARCHING) return 'ricerca griglia…';
    if (confidenceState === STATES.LOST) return 'riga persa, in attesa…';
    return '';
  }
  ...
  function onFrame(videoElement, timestamp) {
    ...
    window.Overlay.updateStatusLabel(
      stateLabelText(confidenceResult.state)
    );
  }
