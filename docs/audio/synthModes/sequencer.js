...
(function () {
  ...
  // --- Vibrazione (feedback aptico su mobile) ---------------------------------
  // Ogni colpo fa vibrare il telefono, sincronizzato col suono.
  // NOTA: la Vibration API è supportata da Android (Chrome/Firefox); Safari
  // su iOS non la espone affatto, quindi su iPhone semplicemente non vibra.
  const VIBRATION_MS = 24;
  ...
  function schedulerTick() {
    ...
    while (nextStepTime < ctx.currentTime + LOOKAHEAD_SECONDS) {
      ...
      if (pattern[nextStepIndex]) {
        ...
        // Vibrazione sincronizzata: il colpo è schedulato NEL FUTURO sul
        // clock audio, quindi la vibrazione va ritardata dello stesso
        // intervallo per coincidere col momento in cui il suono esce davvero.
        if (canVibrate) {
          const delayMs = Math.max(0, (humanizedTime - ctx.currentTime) * 1000);
          setTimeout(() => navigator.vibrate(VIBRATION_MS), delayMs);
        }
      }
