...
(function () {
  ...
  let kickSaturationNode = null; // waveshaper condiviso: calore analogico sul bus kick

  /**
   * Curva di saturazione morbida (tanh normalizzata). drive controlla quanto
   * presto la curva inizia a comprimere: 2-3 = calore gentile, non distorsione.
   */
  function makeSoftSaturationCurve(drive) {
    const samples = 1024;
  ...
  function init() {
    ...
    // Bus di saturazione calda per il kick: un waveshaper con curva tanh
    // morbida. La saturazione arrotonda i transitori e aggiunge armoniche
    // pari/dispari basse — è la differenza tra un colpo "digitale/pulito"
    // e uno "caldo/analogico". Come bonus, le armoniche generate rendono
    // il kick più udibile su speaker piccoli che non riproducono la
    // fondamentale.
    kickSaturationNode = audioContext.createWaveShaper();
    kickSaturationNode.curve = makeSoftSaturationCurve(2.5);
    kickSaturationNode.oversample = '2x';

    const kickBusGain = audioContext.createGain();
    kickBusGain.gain.value = 0.9; // compensazione post-saturazione

    kickSaturationNode.connect(kickBusGain);
    kickBusGain.connect(masterGainNode);

    kickVoices = [];
    for (let i = 0; i < NUM_VOICES; i++) {
      kickVoices.push(buildKickVoice(audioContext, kickSaturationNode));
    }
